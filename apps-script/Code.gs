...
var ONGLET   = 'Clients';
var ENTETES  = ['id','nom','tel','mail','points','offre','notes','optin','maj','visites'];
...
function lireClients_() {
  ...
  return JSON.stringify(rows.map(function(r) {
    ...
    head.forEach(function(h, i) {
      if (h === 'visites') { try { o[h] = r[i] ? JSON.parse(r[i]) : []; } catch(e) { o[h] = []; } }
      else { o[h] = r[i]; }
    });
    return o;
  }));
...
function ecrireClients_(arr) {
  ...
  arr.forEach(function(o) {
    sh.appendRow(ENTETES.map(function(h) {
      if (h === 'visites') return o[h] ? JSON.stringify(o[h]) : '';
      return o[h] != null ? o[h] : '';
    }));
  });
}
